...
LLMエージェントの想定外の稼働による過剰課金を、構造的に防止します。APIキーは `.env` や環境変数ではなく安全なバックエンド（デフォルト: macOS Keychain、1Password等に差し替え可能）に格納し、予算の範囲内でのみ返却します。
...
デフォルトの予算上限は **$0（米ドル）** です。予算を設定するまで、すべてのAPIキー取得はブロックされます。これは意図的な設計であり、明示的な予算決定なしにキーが返されることはありません。
...
すべての予算は**米ドル（USD）**建てで、**暦月**単位で管理されます。デフォルトは `0` です。予算を設定するまでキーは取得できません。
...
ステップ1が失敗した場合、ステップ2-3は実行されません。予算超過時はbantoのAPIを通じてキーにアクセスできないため、エージェントはこれを回避できません。
